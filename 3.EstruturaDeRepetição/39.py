'''Faça um programa que leia dez conjuntos de dois valores, o primeiro representando o número do aluno e o segundo representando a sua altura em centímetros. 
Encontre o aluno mais alto e o mais baixo. Mostre o número do aluno mais alto e o número do aluno mais baixo, junto com suas alturas.'''

numero = []
altura = []

for i in range(1, 11):
  numero.append(int(input('Número do aluno: ')))
  altura.append(float(input('Altura do aluno (cm): ')))
print(f'O maior aluno é o de número {numero[altura.index(max(altura))]} e de altura: {max(altura)} cm')
print(f'O menor aluno é o de número {numero[altura.index(min(altura))]} e de altura: {min(altura)} cm')